...
std::string problem_name = "elastic_contact2";
// purely syntetic
...
int main(int argc, char ** argv)
{
	...
	Tag bndcond = m->CreateTag("BOUNDARY_CONDITION",DATA_REAL,FACE,FACE,7);
	Tag force = m->CreateTag("FORCE",DATA_REAL,CELL,NONE,3);
	Tag contact = m->CreateTag("CONTACT_TYPE",DATA_INTEGER,FACE,NONE,1);
	...
	for(Mesh::iteratorFace it = m->BeginFace(); it != m->EndFace(); ++it)
	{
		...
		if( it->Boundary() )
		{
			...
			//if( false )
			if( c[2] > 1.0-eps || c[2] < 0.0+eps ) //pure roller on z
			{
				bc[0] = 1.0; //alpha perp
				bc[1] = 0.0; //beta
				bc[2] = 0.0; //alpha parallel
				bc[3] = 1.0; //beta
				//right hand side
				bc[4] = 0.0;
				bc[5] = 0.0;
	...
	//rescale mesh to fit quad (0, −5), (35, −5), (35, 5) (0, 5) in Oxy
