...
int main(int argc, char ** argv)
{
	...
	Tag stress_val = m->CreateTag("REFERENCE_STRESS",DATA_REAL,CELL|FACE,NONE,6);
	...
	for(Mesh::iteratorElement it = m->BeginElement(CELL|FACE); it != m->EndElement(); ++it)
	{
		...
		sol(0,0) = -(3*F*nu)/(4*E)*x*y*z;
		sol(1,0) = F/(8*E)*(3*nu*z*(x*x-y*y)-z*z*z);
		sol(2,0) = F/(8*E)*(3*y*z*z + nu*y*(y*y-3*x*x)) + (3*F*(1+nu))/(4*E)*(y-y*y*y/3) + (F*nu)/(4*E)*(3*x*x*y-y);
		
		//     | nx           nz ny | xz
		// N = |    ny     nz    nx | yz
		//     |       nz  ny nx    | zz
		//xx yy zz yz xz xy
		sigma(0,0) = sigma(1,0) = sigma(5,0) = 0; //xx,yy,xy
		sigma(2,0) = 3*F/4*y*z; //zz
		sigma(3,0) = 3*F/8*(1-y*y) + F*nu/(8*(1+nu))*(3*x*x-1);//yz
		sigma(4,0) = 0;//xz
		...
		{
			int n = i;
			sol(2,0) -= 3*F*nu/(pi*pi*pi*E)*minus1/(n*n*n*cosh(n*pi))*cos(n*pi*x)*sinh(n*pi*y);
			sigma(3,0) -= 3*F*nu/(2*pi*pi*(1+nu))*minus1/(n*n*cosh(n*pi))*cos(n*pi*x)*cosh(n*pi*y);
			sigma(4,0) += 3*F*nu/(2*pi*pi*(1+nu))*minus1/(n*n*cosh(n*pi))*sin(n*pi*x)*sinh(n*pi*y);
			minus1 *= -1;
		}
